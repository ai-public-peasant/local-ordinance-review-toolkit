...
이 단계는 법제처 Open API 인증키를 요구하지 않습니다. 이 레포는 ELIS 공개 웹 화면을 조회하므로 기본 실행에는 `OC`가 필요 없습니다.
...
별지·서식 첨부파일은 가능한 범위에서 텍스트로 변환됩니다. 구형 HWP는 `olefile`, HWPX/DOCX/XLSX는 `zipfile`, PDF는 `pypdf`를 사용합니다. 변환 결과가 이상한 경우 원본 첨부파일을 직접 열어 대조해야 합니다.
